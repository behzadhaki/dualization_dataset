...
class DualizationDatasetAPI():
    ...
    def populate_fields(self, midi_folder):
        ...
        self.__summary_dataframe["Test Number"] = [t.split(os.sep)[-1].split(" ")[0].split("[")[-1] for t in test_folders]
        ...
        self.__summary_dataframe["Test Number"] = [t.split(os.sep)[-1].split(" ")[0].split("[")[-1] for t in test_folders]
        self.__summary_dataframe["Style"] = [t.split(os.sep)[-1].split(" ")[-1].split("_")[1].lower() for t in test_folders]
        self.__summary_dataframe["Tempo"] = [float(t.split(os.sep)[-1].split(" ")[-1].split("_")[2]) for t in test_folders]
        self.__summary_dataframe["GMD Drummer"] = [t.split(os.sep)[-1].split(" ")[-1].split("-")[0] for t in test_folders]
        self.__summary_dataframe["GMD Performance Session"] = [t.split(os.sep)[-1].split(" ")[-1].split("-")[1].split("_")[0]
                                        for t in test_folders]
        self.__summary_dataframe["GMD Segment Type"] = [t.split(os.sep)[-1].split(" ")[-1].split("_")[3] for t in test_folders]
        self.__summary_dataframe["GMD Segment Meter"] = [t.split(os.sep)[-1].split(" ")[-1].split("_")[4] for t in test_folders]
        self.__summary_dataframe["Selected 2Bars From Start"] = [int(t.split(os.sep)[-1].split(" ")[-1].split("_")[-1]) for t in
                                         test_folders]
        self.__summary_dataframe["Dualized Midifolder Path"] = [os.path.join(os.path.dirname(t), "dualized") for t in test_folders]
...
class DualizationsForParticipant():
    ...
    def __repr__(self):
        ...
        if self.__TestType is "Three Random Repetitions":
            str += "\n use .rep1, .rep2, .rep3 to access dualizations, \n" \
...
class Pattern:
    ...
    def piano_roll(self, show=True):
        if _HAS_HVO_SEQUENCE:
            fig = self.hvo_sequence.to_html_plot(show_figure=show)
            return fig
        else:
